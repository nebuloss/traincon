...
const { Bookmarks } = await import(path.join(ROOT, 'client/app/Bookmarks.ts'));
...
const { missingKind } = await import(path.join(ROOT, 'client/app/missing.ts'));
...
test('the missing-bookmark card offers no click affordance', async () => {
  ...
  const view = await readFile(path.join(ROOT, 'client/views/WatchView.ts'), 'utf8');
